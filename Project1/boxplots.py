...
def read_data_from_stdin():
    ...
    for line in sys.stdin:
        try:
            ...
            hybrid_time, merge_time = map(float, line.split())
...
def plot_boxplot(hybrid_sort_times, merge_sort_times, output_file="boxplot_graph.png"):
    ...
    data = [hybrid_sort_times, merge_sort_times]
    plt.boxplot(
        data,
        labels=["Hybrid Sort", "Merge Sort"],
        patch_artist=True,
        boxprops=dict(facecolor="lightblue"),
        medianprops=dict(color="red"),
    )

    plt.title("Hybrid Sort vs. Merge Sort Runtime Comparison (t=198, n=1E+7)")
